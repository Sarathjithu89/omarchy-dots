...
Item {
  ...
  // System-sound feedback for incoming notifications. The freedesktop sound
  // theme ships with libcanberra; canberra-gtk-play is fire-and-forget so
  // bursts of notifications overlap naturally instead of cutting each other
  // off. Urgency drives the event name, mirroring the popup styling.
  function playNotificationSound(snapshot) {
    var urgency = Number(snapshot && snapshot.urgency)
    var sound
    if (urgency >= NotificationUrgency.Critical) sound = "dialog-warning"
    else if (urgency <= NotificationUrgency.Low) sound = "message"
    else sound = "message-new-instant"
  ...
  function handleNotification(notification) {
    ...
    service.playNotificationSound(snapshot)
