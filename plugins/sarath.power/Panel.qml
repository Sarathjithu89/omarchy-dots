...
Panel {
  ...
  property bool chargeLimitKnown: false
  readonly property string chargeLimitScript: Quickshell.env("HOME") + "/.config/omarchy/plugins/sarath.power/charge-limit.sh"
  ...
  function setChargeLimit(percent) {
    ...
    if (root.chargeLimitKnown && root.chargeLimit === value) return
    if (limitActionProc.running) return

    // Optimistic for instant feedback; refresh() re-syncs on exit.
    root.chargeLimit = value
    root.chargeLimitKnown = true
    limitActionProc.command = ["pkexec", root.chargeLimitScript, "set", String(value)]
  ...
  Process {
    id: limitProc
    command: [root.chargeLimitScript, "current"]
        ...
        Column {
          ...
          Item {
            ...
            Text {
              id: limitPercent
              textFormat: Text.PlainText
              text: root.chargeLimitKnown ? "Stops at " + root.chargeLimit + "%" : "—"
              color: Qt.darker(root.bar.foreground, 1.4)
              font.family: root.bar.fontFamily
              font.pixelSize: Style.font.caption
              font.bold: true
          ...
          Item {
            width: parent.width
            implicitHeight: root.chargeLimitKnown
              ? limitSlider.implicitHeight + Style.spacing.controlGap
              : 0
          ...
          Row {
            ...
            readonly property var presets: [60, 70, 80, 90, 100]
            readonly property real cellWidth: (width - spacing * (presets.length - 1)) / presets.length

            Repeater {
              model: limitPresetRow.presets
              Button {
                required property int modelData
                width: limitPresetRow.cellWidth
                text: modelData === 100 ? "Full" : modelData + "%"
                fontSize: Style.font.bodySmall
                foreground: root.bar.foreground
                fontFamily: root.bar.fontFamily
                horizontalPadding: Style.spacing.controlPaddingX
                verticalPadding: Style.spacing.controlPaddingY + Style.space(2)
                bordered: true
                active: root.chargeLimitKnown && root.chargeLimit === modelData
                onClicked: root.setChargeLimit(modelData)
                onHovered: function(h) {
                  if (h) root.cursorActive = false
                }
              }
            }
          }
          ...
          Text {
            ...
            textFormat: Text.PlainText
            text: "Charging stops when the battery reaches this level. Full disables the limit."
