...
Item {
  ...
  function show(iconName, rawMessage, rawValue, rawMax, rawProgressText, rawDuration) {
    ...
    // Volume feedback blip. Fire-and-forget through the freedesktop sound
    // theme; skipped on mute so a silenced system stays quiet. Matches both
    // the omarchy audio commands' icon names ("volume-high"/"volume-muted")
    // and the audio panel's glyph icons.
    var volumeIcon = next.iconKey.indexOf("volume") === 0 ||
      next.iconKey === "" || next.iconKey === "" || next.iconKey === ""
    var mutedIcon = next.iconKey === "volume-muted" || next.iconKey === "muted" || next.iconKey === ""
